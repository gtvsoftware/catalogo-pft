...
export default function SeriesListPage() {
  ...
  const columns: ColumnDef<Serie>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <HeaderColumnSorting column={column} content="Grupo" />
      ),
      cell: ({ row }) => {
        const grupoNome = row.getValue('grupoNome') as string
        return <div>{grupoNome}</div>
      }
    },
    {
    ...
    {
      accessorKey: '_count.variacoes',
      header: 'Variações',
      cell: ({ row }) => {
        const count = row.original._count.variacoes
        return (
        ...
        return (
          <div className="flex gap-2">
            <Button
              ...
              size="sm"
              onClick={() =>
                router.push(`/cadastros/series/${row.original.id}`)
              }
            >
