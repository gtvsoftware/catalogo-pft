...
export default function Page() {
  ...
  const searchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (filters.tipo_produto) filterArray.push(`tipo_produto:${filters.tipo_produto}`)
      if (filters.tipo_embalagem) filterArray.push(`tipo_embalagem:${filters.tipo_embalagem}`)

      if (filterArray.length > 0) {
        params.append('filter_by', filterArray.join(' && '))
      }

      const response = await fetch(`http://localhost:8108/collections/conjuntos_comerciais/documents/search?${params}`, {
        headers: {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Busque por mais de 6.000 produtos usando busca inteligente e filtros avançados
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select
              ...
              onChange={(e) => setFilters(prev => ({ ...prev, tipo_produto: e.target.value }))}
              className="px-3 py-2 border border-gray-300 rounded-md"
            >
            ...
            <select
              ...
              onChange={(e) => setFilters(prev => ({ ...prev, tipo_embalagem: e.target.value }))}
              className="px-3 py-2 border border-gray-300 rounded-md"
            >
            ...
            <select
              ...
              className="px-3 py-2 border border-gray-300 rounded-md"
            >
              <option value="preco_venda_sugerido:asc">Preço: Menor → Maior</option>
              <option value="preco_venda_sugerido:desc">Preço: Maior → Menor</option>
        ...
        {results && !loading && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            {results.facet_counts && (
              <div className="lg:col-span-1">
                <div className="bg-white rounded-lg shadow-sm border p-4">
                  <h3 className="font-semibold text-gray-900 mb-4">Filtros Disponíveis</h3>
            ...
            {/* Products Grid */}
            <div className={`${results.facet_counts ? 'lg:col-span-3' : 'lg:col-span-4'}`}>
              <div className="mb-4 flex justify-between items-center">
                <p className="text-gray-600">
                  <strong>{results.found.toLocaleString()}</strong> produtos encontrados
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {results.hits.map((hit) => (
                  <div key={hit.document.id} className="bg-white rounded-lg shadow-sm border hover:shadow-md transition-shadow">
                    <div className="p-4">
                      ...
                      <div className="text-sm text-gray-600 space-y-1 mb-3">
                        <p><span className="font-medium">Tipo:</span> {hit.document.produto_base_nome}</p>
                        {hit.document.cor && (
                          <p><span className="font-medium">Cor:</span> {hit.document.cor}</p>
                        )}
                        {hit.document.altura_cm && (
                          <p><span className="font-medium">Altura:</span> {hit.document.altura_cm}cm</p>
                        )}
                        {hit.document.numero_hastes && (
                          <p><span className="font-medium">Hastes:</span> {hit.document.numero_hastes}</p>
                        )}
                        <p><span className="font-medium">Embalagem:</span> {hit.document.tipo_embalagem}</p>
                        <p><span className="font-medium">Código:</span> {hit.document.codigo_veiling}</p>
                      </div>
        ...
        {!results && !loading && (
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <h3 className="font-semibold text-gray-900 mb-4">Exemplos de busca:</h3>
