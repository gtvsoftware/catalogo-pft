...
function Calendar({
  ...
  const _monthsClassName = cn('relative flex gap-8', props.monthsClassName)
  ...
  const _navClassName = cn(
    'absolute w-full flex items-start',
    props.navClassName
  )
  ...
  return (
    <DayPicker
      ...
      classNames={{
        day_today: 'bg-primary-600 text-white rounded-full!',
        week_number:
          'font-bold text-sm w-[32px] mr-2 h-[32px] text-primary-500 flex items-center justify-center rounded-md',
      ...
      components={{
        Chevron: ({ orientation }) => {
          return (
            <Icon
              icon={orientation === 'left' ? 'chevron-left' : 'chevron-right'}
              className="h-4 w-4"
            />
          )
        },
